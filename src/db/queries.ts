...
export async function deleteMemory(memoryId: string): Promise<boolean> {
  const result = await query("DELETE FROM memories WHERE id = $1", [memoryId]);
...
function buildVisibilityScope(
  ...
  if (opts.agentId) {
    ...
    if (opts.userId) {
      ...
      scopeFilters.push(
        `(agent_id = $${agentParam} AND user_id = $${userParam})`,
      );
...
export async function searchByKeyword(
  ...
  const { conditions, params } = buildVisibilityScope(opts, 3, [
    opts.query,
    limit,
  ]);
...
function buildFactVisibilityScope(
  opts: Pick<
    FactRecallOptions,
    "agentId" | "userId" | "taskId" | "includeShared"
  >,
  startParamIdx: number,
  startParams: unknown[],
): VisibilityResult {
  ...
  if (opts.agentId) {
    ...
    if (opts.userId) {
      ...
      params.push(opts.userId);
      scopeFilters.push(
        `(m.agent_id = $${agentParam} AND m.user_id = $${userParam})`,
      );
 ...
 */
export async function getFactIdsForMemory(memoryId: string): Promise<string[]> {
...
export async function searchFactsByEntityEmbedding(
  ...
  const result = await query<ScoredFact>(sql, [
    embeddingStr,
    limit,
    entityFactIds,
  ]);
 ...
 */
export async function getFactById(factId: string): Promise<ScoredFact | null> {
  const result = await query<FactRow>(
    `SELECT f.*, m.created_at as memory_created_at
     FROM memory_facts f
     JOIN memories m ON f.memory_id = m.id
