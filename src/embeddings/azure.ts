...
export class AzureEmbeddingProvider implements EmbeddingProvider {
  ...
  constructor(endpoint: string, apiKey: string) {
    if (!endpoint) {
      throw new Error('Azure embedding endpoint required. Set AZURE_EMBEDDING_ENDPOINT.');
    }
    if (!apiKey) {
  ...
  async init(): Promise<void> {
    console.log('[memforge] Azure embedding provider initialized (text-embedding-3-large, 3072 dims)');
  }
  ...
  async embed(text: string): Promise<number[]> {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Azure embedding API error (${response.status}): ${error}`);
    }
