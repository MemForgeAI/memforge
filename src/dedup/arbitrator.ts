...
const NEGATION_PAIRS = [
  [
    /\b(is|are|was|were)\s+(?:a\s+)?(\w+)/i,
    /\b(is|are|was|were)\s+not\s+(?:a\s+)?(\w+)/i,
  ],
  [
    /\b(likes?|loves?|enjoys?|prefers?)\b/i,
    /\b(hates?|dislikes?|avoids?|doesn'?t\s+like)\b/i,
  ],
  [/\b(always)\b/i, /\b(never)\b/i],
  [
    /\b(vegetarian|vegan)\b/i,
    /\b(meat|steak|burger|chicken|pork|bacon)\b.*\b(loves?|enjoys?|prefers?|eats?)\b/i,
  ],
