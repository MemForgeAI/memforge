...
function candidate(overrides: Partial<SimilarityCandidate> & { content: string }): SimilarityCandidate {
  return {
    id: 'existing-1',
    similarity: 0.88,
    memoryType: 'semantic',
  ...
  describe('elaboration detection', () => {
    ...
    it('detects when existing is more detailed (reverse elaboration)', () => {
      const result = arbitrate(
        'User likes coffee',
        candidate({
          content: 'User likes coffee, particularly Ethiopian single-origin dark roast from Blue Bottle',
        }),
      );
  ...
  describe('new memory (no conflict)', () => {
    ...
    it('returns new when neither is elaboration nor contradiction', () => {
      const result = arbitrate(
        ...
        candidate({ content: 'User has experience with Python and TypeScript' }),
      );
      expect(result.action).toBe('new');
    });
  });
