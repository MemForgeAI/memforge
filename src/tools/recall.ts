...
import {
  searchByEmbedding, searchByKeyword, getMemory,
  searchFactsByEmbedding, searchFactsByKeyword, hasAnyFacts,
...
import { extractEntities, extractQueryEntities } from '../extraction/extractor.js';
import {
  traverseFromEntity, findEntitiesForMemory, findMemoriesByEntity,
  findFactsByEntity, findEntitiesForFact,
} from '../graph/age.js';
import type { Config } from '../config.js';
import { scorePairs, isReady as isRerankerReady } from '../reranker/crossEncoder.js';
...
async function recallFromFacts(
  ...
  if (allFacts.length === 0) {
    return { context: '', total_tokens: 0, memories_used: 0, _factMemoryIds: [] };
  }
        ...
        try {
          ...
          if (candidateIds.length > 0) {
            const ranked = await searchFactsByEntityEmbedding(
              candidateIds, queryEmbedding, MAX_PERSON_FACTS,
            );
  ...
  try {
    ...
    if (uniqueEntityFactIds.length > 0) {
      const rankedGraphFacts = await searchFactsByEntityEmbedding(
        uniqueEntityFactIds, queryEmbedding, MAX_GRAPH_ADDITIONS,
      );
    ...
    if (graphAdded > 0) {
      console.log(`[memforge] Graph enrichment added ${graphAdded} ranked facts`);
    }
  } catch (err) {
    console.warn('[memforge] Fact graph enrichment failed:', (err as Error).message);
  }
      ...
      for (const pe of personEntities) {
        try {
          ...
          if (candidateIds.length > 0) {
            ...
            if (ranked.length > 0) {
              console.log(`[memforge] Aggregation: added ${ranked.length} ranked person-entity facts (cap 30)`);
            }
          }
        } catch {
          // Graph failures are non-fatal
...
Return JSON: {"complete": true/false, "follow_up_queries": ["query1", "query2"]}`;
  ...
  const response = await callChatLLM(GAP_SYSTEM_PROMPT, userPrompt, config, 15000);
  ...
  try {
    ...
    return {
      ...
      followUpQueries: Array.isArray(parsed.follow_up_queries)
        ? parsed.follow_up_queries.slice(0, 3).filter((q): q is string => typeof q === 'string' && q.length > 0)
        : [],
    };
  } catch {
...
async function recallAggregationWithRefinement(
  ...
  if (!gaps || gaps.complete || gaps.followUpQueries.length === 0) {
    if (gaps?.complete) {
      ...
      console.log('[memforge] Agentic recall: gap analysis failed, using pass-1 results');
    }
  ...
  console.log(`[memforge] Agentic recall: pass-2 added ${pass2Added} new facts`);
...
async function recallMultiHop(
  ...
  const memoryPromise = recallFromMemories(
    input, originalEmbedding, config, memoryBudget, recencyWeight,
  ).catch(() => null);
  ...
  const factContext = factContextParts.length > 0
...
export async function recall(
  ...
  if (factsExist) {
    ...
    switch (classification.type) {
      case 'AGGREGATION': {
        const factBudget = Math.floor(tokenBudget * 0.60);
        const memoryBudget = Math.floor(tokenBudget * 0.40);
        ...
        const factPromise = config.agenticRecallEnabled
          ? recallAggregationWithRefinement(
              input, queryEmbedding, embedder, config, factBudget, recencyWeight,
            )
          : recallAggregationFacts(
              input, queryEmbedding, config, factBudget, recencyWeight,
            );

        // Memory search runs in parallel (not blocked by LLM gap analysis)
        const [factResult, memoryResult] = await Promise.all([
          factPromise,
          recallFromMemories(input, queryEmbedding, config, memoryBudget, recencyWeight),
        ]);
      ...
      case 'MULTI_HOP': {
        ...
        if (subQueries.length >= 2) {
          console.log(`[memforge] Multi-hop decomposed into ${subQueries.length} sub-queries: ${JSON.stringify(subQueries)}`);
          return recallMultiHop(
            input, queryEmbedding, subQueries, embedder, config,
            tokenBudget, recencyWeight,
          );
        }
        // Decomposition failed — fall through to DIRECT
        console.log('[memforge] Multi-hop decomposition failed, falling back to DIRECT');
        break;
      }
  ...
  return recallFromMemories(
    input, queryEmbedding, config, tokenBudget, recencyWeight,
  );
}

/** Combine fact and memory results, handling empty cases */
function combineResults(factResult: RecallResult, memoryResult: RecallResult): RecallResult {
...
function extractKeywords(content: string): Set<string> {
  const stopwords = new Set([
    ...
    'uses', 'using', 'used', 'alex', 'team', 'project', 'prefers',
  ]);
  const words = content.toLowerCase().split(/\W+/).filter(
...
function filterTemporalDuplicates(memories: ScoredMemory[]): ScoredMemory[] {
  ...
  for (let i = 0; i < memories.length; i++) {
    ...
    const hasSupersessionLanguage = SUPERSESSION_PATTERNS.some((p) => p.test(memI.content));
