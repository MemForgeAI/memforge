...
import { upsertEntity, upsertRelationship, linkMemoryToEntity, linkFactToEntity } from '../graph/age.js';
...
export async function remember(
  ...
  if (config.llmExtractionEnabled && config.azureOpenaiEndpoint && config.azureOpenaiKey && contentWords >= 5) {
    try {
      ...
      console.warn('[memforge] Memory Card extraction failed:', (err as Error).message);
    }
  }
  ...
  const embedding = textToEmbed === input.content
    ? rawEmbedding
  ...
  if (exactDupes.length > 0) {
    ...
    await touchMemory(existing.id, Math.max(existing.confidence, input.importance ?? 0.5));
  ...
  for (const entity of extraction.entities) {
    try {
      ...
      console.warn(`[memforge] Graph entity upsert failed for "${entity.name}":`, (err as Error).message);
    }
  }
  ...
  for (const rel of extraction.relationships) {
    try {
      await upsertRelationship(
        rel.from.name, rel.from.type,
        rel.to.name, rel.to.type,
        rel.relation,
      );
    } catch (err) {
      console.warn(`[memforge] Graph relationship failed:`, (err as Error).message);
    }
  }
  ...
  if (card?.atomic_facts && card.atomic_facts.length > 0) {
    const sigMap = distributeSignatures(card.atomic_facts, card.query_signatures);
    ...
    for (let i = 0; i < card.atomic_facts.length; i++) {
      ...
      try {
        ...
        console.warn(`[memforge] Fact storage failed for "${atomicFact}":`, (err as Error).message);
      }
    }
  }
