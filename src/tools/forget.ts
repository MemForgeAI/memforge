...
export async function forget(input: ForgetInput): Promise<ForgetResult> {
  ...
  try {
    ...
    console.warn(`[memforge] Graph cleanup failed for memory ${input.memory_id}:`, (err as Error).message);
  }
  ...
  try {
    ...
    console.warn(`[memforge] Fact graph cleanup failed for memory ${input.memory_id}:`, (err as Error).message);
  }
