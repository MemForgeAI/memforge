...
export function extractEntities(content: string): ExtractionResult {
  ...
  entities.push(
    ...extractByPatterns(content, LOCATION_PATTERNS, "location", 0.6),
  );
  entities.push(
    ...extractByPatterns(content, ORG_PATTERNS, "organization", 0.6),
  );
  entities.push(
    ...extractByPatterns(content, PREFERENCE_PATTERNS, "preference", 0.5),
  );
  ...
  for (const entity of entities) {
    ...
    if (
      !existing ||
      (typeRank[entity.type] ?? 0) > (typeRank[existing.type] ?? 0)
    ) {
