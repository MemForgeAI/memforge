...
export async function createMemoryCard(
  ...
  const prompt = MEMORY_CARD_PROMPT.replace('{OBSERVATION}', observation.replace(/"/g, '\\"'));
  ...
  if (!response.ok) {
    ...
    throw new Error(`Memory card extraction failed (${response.status}): ${error}`);
  }
    ...
    if (item.type === 'message') {
      for (const content of item.content ?? []) {
        if (content.type === 'output_text' && content.text) {
          try {
            const card = JSON.parse(content.text) as Omit<MemoryCard, 'original'>;
            ...
            if (match) {
              try {
                const card = JSON.parse(match[0]) as Omit<MemoryCard, 'original'>;
                return { ...card, original: observation };
              } catch {
                // Try to repair truncated JSON
                const repaired = repairJson(match[0]);
                if (repaired) {
                  const card = JSON.parse(repaired) as Omit<MemoryCard, 'original'>;
                  return { ...card, original: observation };
                }
              }
            }
