...
describe('extractEntities', () => {
  ...
  describe('location extraction', () => {
    it('extracts cities after "in"', () => {
      ...
      expect(locations.some((l) => l.name.includes('San Francisco'))).toBe(true);
    });
  ...
  describe('organization extraction', () => {
    ...
    it('extracts companies with Inc/Corp suffix', () => {
      const result = extractEntities('Partnering with Acme Corp on the project');
      const orgs = result.entities.filter((e) => e.type === 'organization');
  ...
  describe('relationships', () => {
    it('creates relationships between co-occurring entities', () => {
      const result = extractEntities('Dr. Jane Doe works at Google in San Francisco');
      expect(result.relationships.length).toBeGreaterThan(0);
    });

    it('infers WORKS_AT between person and organization', () => {
      const result = extractEntities("User's name is Alice and works at Microsoft Corp");
      const worksAt = result.relationships.filter((r) => r.relation === 'WORKS_AT');
  ...
  describe('deduplication', () => {
    it('does not return duplicate entities', () => {
      const result = extractEntities('User lives in Tokyo and wants to travel to Tokyo');
  ...
  describe('edge cases', () => {
    it('returns empty for text with no entities', () => {
      const result = extractEntities('the quick brown fox jumps over the lazy dog');
