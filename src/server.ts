  ...
  server.tool(
    ...
    `Store something you've learned about the user or task. Call this when you observe a preference, fact, event, or procedure that might be useful in future conversations. MemForge auto-classifies, embeds, deduplicates, and stores it. Examples: "User prefers window seats", "User booked flight to Tokyo on Jan 15", "When booking for this user, always check loyalty points first".`,
    {
      ...
      user_id: z
        .string()
        .optional()
        .describe("Whose memory this is (optional)"),
      agent_id: z
        .string()
        .optional()
        .describe(
          'Which agent is storing this (optional, defaults to "default")',
        ),
      importance: z
        .number()
        .min(0)
        ...
        .optional()
        .describe(
          "How important is this? 0-1, defaults to 0.5. Use 1.0 for safety-critical info like allergies.",
        ),
      shared: z
        .boolean()
        .optional()
        .describe(
          "Make visible to other agents serving this user? Defaults to false.",
        ),
      task_id: z
        .string()
        .optional()
        .describe(
          "Associate with a specific task for task-scoped sharing (optional)",
        ),
    },
    async (params) => {
  ...
  server.tool(
    ...
    `Retrieve relevant context for your current task. Call this BEFORE responding when you need to remember something about the user, their preferences, past interactions, or procedures. Returns a compiled, token-budgeted context document grouped by type (FACTS / HISTORY / PROCEDURES), not a raw list. Examples: "What do I know about this user's travel preferences?", "Any procedures for booking flights for this user?", "What happened in the last session?"`,
    {
      query: z
        .string()
        .describe(
          "What you need to know — describe your current task or question in natural language",
        ),
      user_id: z
        .string()
        .optional()
        .describe("Whose memories to search (optional)"),
      agent_id: z
        .string()
        .optional()
        .describe('Which agent is recalling (optional, defaults to "default")'),
      task_id: z
        .string()
        .optional()
        .describe(
          "Include task-shared memories for this task ID. Useful for multi-agent task coordination.",
        ),
      token_budget: z
        .number()
        .optional()
        .describe(
          "Max tokens to return. Defaults to 2000. Use less for focused context, more for comprehensive.",
        ),
      recency_weight: z
        .number()
        .min(0)
        ...
        .optional()
        .describe(
          '0-1, how much to favor recent memories. High (0.8+) for "what just happened?", low (0.2) for long-term preferences. Defaults to 0.5.',
        ),
      include_shared: z
        .boolean()
        .optional()
        .describe(
          "Include memories shared by other agents? Useful for multi-agent coordination.",
        ),
    },
    async (params) => {
  ...
  server.tool(
    ...
    `Analyze recent memories and extract higher-level insights. Call this periodically to consolidate episodic experiences into semantic knowledge. For example, if you've had multiple conversations about travel, reflect might produce "User frequently discusses travel to Japan — likely a strong interest." Returns a list of newly created insight memories.`,
    {
      user_id: z
        .string()
        .optional()
        .describe("Whose memories to reflect on (optional)"),
      agent_id: z
        .string()
        .optional()
        .describe(
          'Which agent is reflecting (optional, defaults to "default")',
        ),
      lookback_hours: z
        .number()
        .optional()
        .describe(
          "How far back to look for memories to consolidate. Defaults to 48 hours.",
        ),
    },
    async (params) => {
...
async function main(): Promise<void> {
  ...
  console.log(
    `[memforge] Config loaded: embedding=${config.embeddingProvider}, port=${config.port}`,
  );
  ...
  console.log(
    `[memforge] Embedding provider ready: ${embedder.name} (${embedder.dimension} dims)`,
  );
  ...
  if (config.rerankerEnabled) {
    try {
      ...
      console.warn(
        "[memforge] Cross-encoder failed to load, re-ranking disabled:",
  ...
  app.listen(config.port, () => {
    console.log(
      `[memforge] MCP server listening on http://localhost:${config.port}/mcp`,
    );
    console.log(
      `[memforge] Health check: http://localhost:${config.port}/health`,
    );
  });
