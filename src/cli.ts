...
async function cmdRemember(
  ...
  console.log(`Storing memory: "${content.slice(0, 60)}${content.length > 60 ? '...' : ''}"`);
...
async function cmdRecall(
  ...
  console.log(`Recalling: "${queryText.slice(0, 60)}${queryText.length > 60 ? '...' : ''}"`);
  console.log(`  budget: ${input.token_budget ?? config.defaultTokenBudget} tokens`);
...
async function cmdStatus(): Promise<void> {
  ...
  try {
    const versionResult = await query<{ version: string }>(
      'SELECT version()',
    );
  ...
  console.log(`Conflicts: ${conflictResult.rows[0]?.count ?? 0} (${conflictResult.rows[0]?.pending ?? 0} pending)`);

  // Recent memory
  const recentResult = await query<{ content: string; memory_type: string; created_at: string }>(
  ...
  if (recentResult.rows.length > 0) {
    ...
    for (const row of recentResult.rows) {
      const excerpt = row.content.length > 60
...
async function main(): Promise<void> {
  ...
  try {
    ...
    die(`Config error: ${(err as Error).message}\nMake sure DATABASE_URL is set.`);
  }
  ...
  if (command === 'reflect') {
    ...
    console.log(`Reflecting on memories from last ${input.lookback_hours} hours...`);
    const start = Date.now();
    ...
    if (result.insights_created.length === 0) {
      console.log(`No new insights. Analyzed ${result.memories_analyzed} memories.`);
    } else {
      console.log(`Created ${result.insights_created.length} insights from ${result.memories_analyzed} memories:`);
      for (const insight of result.insights_created) {
        const excerpt = insight.content.length > 80
  ...
  if (!positionalArg) {
    die(`Missing argument. Usage: memforge-cli ${command} <${command === 'forget' ? 'memory-id' : 'text'}>`);
  }
