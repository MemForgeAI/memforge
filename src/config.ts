...
const PROVIDER_DEFAULTS: Record<string, { dedup: number; conflict: number }> = {
  ...
  openai: { dedup: 0.95, conflict: 0.8 },
  azure: { dedup: 0.95, conflict: 0.8 },
};
...
const ConfigSchema = z.object({
  ...
  queryDecompositionEnabled: z
    .string()
    .transform((v) => v !== "false")
    .default("true"),
  agenticRecallEnabled: z
    .string()
    .transform((v) => v !== "false")
    .default("false"),
...
export function loadConfig(): Config {
  ...
  const defaults =
    PROVIDER_DEFAULTS[raw.embeddingProvider] ?? PROVIDER_DEFAULTS["local"]!;
