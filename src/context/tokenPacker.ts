...
export function packContext(
  ...
  const highConfidence = memories.filter(
    (m) => m.score >= HIGH_CONFIDENCE_SCORE,
  );
  ...
  if (tokensUsed < effectiveBudget) {
    const mediumConfidence = memories.filter(
      (m) =>
        m.score >= LOW_CONFIDENCE_SCORE &&
        m.score < HIGH_CONFIDENCE_SCORE &&
        !packed.has(m.id),
    );
...
export function packFactContext(
  ...
  if (tokensUsed < effectiveBudget) {
    const mediumConfidence = facts.filter(
      (f) =>
        f.score >= LOW_CONFIDENCE_SCORE &&
        f.score < HIGH_CONFIDENCE_SCORE &&
        !packed.has(f.id),
    );
