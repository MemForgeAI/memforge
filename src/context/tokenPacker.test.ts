...
describe("packContext", () => {
  ...
  it("formats memories as bullet points", () => {
    const memories = [makeMemory("User likes sushi", "semantic", 0.9)];
...
describe("two-pass packing", () => {
  it("packs high-confidence memories before medium-confidence ones", () => {
    ...
    expect(result.context).toContain(
      "Medium confidence recency-boosted memory",
    );
  });
  ...
  it("excludes memories with score below LOW_CONFIDENCE_SCORE (0.2)", () => {
    const memories = [
      ...
      makeMemory(
        "Noise low-score memory that should be excluded",
        "semantic",
        0.1,
      ),
    ...
    expect(result.context).not.toContain(
      "Another noise memory below threshold",
    );
  });
  ...
  it("fills budget with high-confidence first, then medium-confidence", () => {
    ...
    expect(result.context).toContain(
      "another high score item ranked third now",
    );
    expect(result.memoriesIncluded).toBe(2);
  });
  ...
  it("does not pack memory with score just below 0.2", () => {
    const memories = [makeMemory("Below threshold memory", "semantic", 0.19)];
  ...
  it("skips pass 2 when budget is exhausted after pass 1", () => {
    ...
    const memories = [
      ...
      makeMemory(
        "Medium confidence memory that should definitely not fit in the remaining budget at all",
        "episodic",
        0.3,
      ),
    ];
    ...
    expect(result.context).not.toContain(
      "Medium confidence memory that should definitely not fit",
    );
  });
...
describe("packFactContext", () => {
  ...
  it("two-pass: packs high-confidence facts before medium-confidence", () => {
    ...
    expect(result.context).toContain(
      "Medium confidence fact about preferences",
    );
  });
