 ...
 */
const FACT_WEIGHT_SIMILARITY = 0.70;
...
export function rankFacts(
  ...
  const now = Date.now();
  const lambda = DEFAULT_RECENCY_LAMBDA * (0.5 + recencyWeight);
  const isTemporalQuery = query ? TEMPORAL_QUERY_PATTERNS.some((p) => p.test(query)) : false;
  ...
  const isRecencyQuery = query ? /\b(recently|lately|latest|most\s+recent)\b/i.test(query) : false;
  if (isRecencyQuery) {
    ...
    const factsWithDates = filtered
      .map((f) => {
        ...
        const ts = new Date(+match[3]!, MONTHS[match[2]!.toLowerCase()]!, +match[1]!).getTime();
        return { fact: f, ts };
      })
